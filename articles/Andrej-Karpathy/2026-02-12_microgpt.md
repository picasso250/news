...
Where to find it:
This GitHub gist has the full source code: [microgpt.py](https://gist.github.com/karpathy/8627fe009c40f57531cb18360106ce95) It’s also available on this web page: [https://karpathy.ai/microgpt.html](https://karpathy.ai/microgpt.html) Also available as a [Google Colab notebook](https://colab.research.google.com/drive/1vyN5zo6rqUp_dYNbT4Yrco66zuWCZKoN?usp=sharing)
...
Every time you do math with Value objects (add, multiply, etc.), the result is a new Value that remembers its inputs ( _children ) and the local derivative of that operation ( _local_grads ). For example, __mul__ records that ∂ ( a ⋅ b ) ∂ a = b ∂ ( a ⋅ b ) ∂ a = b and ∂ ( a ⋅ b ) ∂ b = a ∂ ( a ⋅ b ) ∂ b = a . The full set of lego blocks:
| Operation | Forward | Local gradients |
|---|---|---|
| a + b | a + b a + b | ∂ ∂ a = 1 , ∂ ∂ b = 1 ∂ ∂ a = 1 , ∂ ∂ b = 1 |
| a * b | a ⋅ b a ⋅ b | ∂ ∂ a = b , ∂ ∂ b = a ∂ ∂ a = b , ∂ ∂ b = a |
| a ** n | a n a n | ∂ ∂ a = n ⋅ a n − 1 ∂ ∂ a = n ⋅ a n − 1 |
| log(a) | ln ( a ) ln ⁡ ( a ) | ∂ ∂ a = 1 a ∂ ∂ a = 1 a |
| exp(a) | e a e a | ∂ ∂ a = e a ∂ ∂ a = e a |
| relu(a) | max ( 0 , a ) max ( 0 , a ) | ∂ ∂ a = 1 a > 0 ∂ ∂ a = 1 a > 0 |
The backward() method walks this graph in reverse topological order (starting from the loss, ending at the parameters), applying the chain rule at each step. If the loss is L L and a node v v has a child c c with local gradient ∂ v ∂ c ∂ v ∂ c , then:
∂ L ∂ c + = ∂ v ∂ c ⋅ ∂ L ∂ v ∂ L ∂ c + = ∂ v ∂ c ⋅ ∂ L ∂ v
...
We kick things off by setting self.grad = 1 at the loss node, because ∂ L ∂ L = 1 ∂ L ∂ L = 1 : the loss’s rate of change with respect to itself is trivially 1. From there, the chain rule just multiplies local gradients along every path back to the parameters.
Note the += (accumulation, not assignment). When a value is used in multiple places in the graph (i.e. the graph branches), gradients flow back along each branch independently and must be summed. This is a consequence of the multivariable chain rule: if c c contributes to L L through multiple paths, the total derivative is the sum of contributions from each path.
After backward() completes, every Value in the graph has a .grad containing ∂ L ∂ v ∂ L ∂ v , which tells us how the final loss would change if we nudged that value.
...
softmax converts a vector of raw scores (logits), which can range from − ∞ − ∞ to + ∞ + ∞ , into a probability distribution: all values end up in [ 0 , 1 ] [ 0 , 1 ] and sum to 1. We subtract the max first for numerical stability (it doesn’t change the result mathematically, but prevents overflow in exp ).
...
Over 1,000 steps the loss decreases from around 3.3 (random guessing among 27 tokens: − log ( 1 / 27 ) ≈ 3.3 − log ⁡ ( 1 / 27 ) ≈ 3.3 ) down to around 2.37. Lower is better, and the lowest possible is 0 (perfect predictions), so there’s still room to improve, but the model is clearly learning the statistical patterns of names.
## Inference
...
mathjax disqus comments Please enable JavaScript to view the [comments powered by Disqus.](http://disqus.com/?ref_noscript) <h2 class="footer-heading">Andrej Karpathy blog</h2> Andrej Karpathy blog <li><a href="mailto:"></a></li> [karpathy](https://github.com/karpathy) [karpathy](https://twitter.com/karpathy)
