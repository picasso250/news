...
# HTML_TAG_START Welcome FLUX.2 - BFL’s new open image generation model 🤗 HTML_TAG_END
Published November 25, 2025 [Update on GitHub](https://github.com/huggingface/blog/blob/main/flux-2.md) [Upvote 180](/login?next=%2Fblog%2Fflux-2) +174 [YiYi Xu YiYiXu Follow](/YiYiXu) [Daniel Gu dg845 Follow](/dg845) [Sayak Paul sayakpaul Follow](/sayakpaul) [Alvaro Somoza OzzyGT Follow](/OzzyGT) [Dhruv Nair dn6 Follow](/dn6) [Aritra Roy Gosthipaty ariG23498 Follow](/ariG23498) [Linoy Tsaban linoyts Follow](/linoyts) [Apolinário from multimodal AI art multimodalart Follow](/multimodalart) HTML_TAG_START
[HTML_TAG_START FLUX.2: A Brief Introduction HTML_TAG_END](#flux2-a-brief-introduction) [HTML_TAG_START Text encoder HTML_TAG_END](#text-encoder) [HTML_TAG_START DiT HTML_TAG_END](#dit) [HTML_TAG_START Misc HTML_TAG_END](#misc) [HTML_TAG_START Inference With Diffusers HTML_TAG_END](#inference-with-diffusers) [HTML_TAG_START Installation and Authentication HTML_TAG_END](#installation-and-authentication) [HTML_TAG_START Regular Inference HTML_TAG_END](#regular-inference) [HTML_TAG_START Resource-constrained HTML_TAG_END](#resource-constrained) [HTML_TAG_START Multiple images as reference HTML_TAG_END](#multiple-images-as-reference) [HTML_TAG_START Advanced Prompting HTML_TAG_END](#advanced-prompting) [HTML_TAG_START LoRA fine-tuning HTML_TAG_END](#lora-fine-tuning) [HTML_TAG_START Memory optimizations for fine-tuning HTML_TAG_END](#memory-optimizations-for-fine-tuning) [HTML_TAG_START Resources HTML_TAG_END](#resources) FLUX.2 is the recent series of image generation models from Black Forest Labs, preceded by the [Flux.1](https://huggingface.co/collections/black-forest-labs/flux1) series. It is an entirely new model with a new architecture and pre-training done from scratch! In this post, we discuss the key changes introduced in FLUX.2, performing inference with it under various setups, and LoRA fine-tuning.
🚨 FLUX.2 is not meant to be a drop-in replacement of FLUX.1, but a new image generation and editing model.
Table of contents
[FLUX.2 introduction](#flux2-a-brief-introduction) [Inference with Diffusers](#inference-with-diffusers) [Advanced Prompting](#advanced-prompting) [LoRA fine-tuning](#lora-fine-tuning)
...
### Misc
A new Autoencoder aka AutoencoderKLFlux2 Better way to incorporate resolution-dependent timestep schedules
...
repo_id = "black-forest-labs/FLUX.2-dev" pipe = Flux2Pipeline.from_pretrained(repo_id, torch_dtype=torch.bfloat16)
pipe.enable_model_cpu_offload()
image = pipe(
prompt= "dog dancing near the sun" ,
num_inference_steps= 50 , # 28 is a good trade-off guidance_scale= 4 ,
height= 1024 ,
width= 1024 ).images[ 0 ]
...
pipe = Flux2Pipeline.from_pretrained(path, torch_dtype=torch.bfloat16) + pipe.transformer.set_attention_backend("_flash_3_hub") pipe.enable_model_cpu_offload()
image = pipe(
prompt="dog dancing near the sun",
...
pipe.enable_model_cpu_offload()
prompt = "Realistic macro photograph of a hermit crab using a soda can as its shell, partially emerging from the can, captured with sharp detail and natural colors, on a sunlit beach with soft shadows and a shallow depth of field, with blurred ocean waves in the background. The can has the text `BFL Diffusers` on it and it has a color gradient that start with #FF5733 at the top and transitions to #33FF57 at the bottom." image = pipe(
prompt=prompt,
...
Local + remote
Due to the modular design of a Diffusers pipeline, we can isolate modules and work with them in sequence. We decouple the text encoder and deploy it to an [Inference Endpoint](https://endpoints.huggingface.co/) . This helps us with freeing up the VRAM usage for the DiT and VAE only.
...
Unfold from diffusers import Flux2Pipeline, Flux2Transformer2DModel from diffusers import BitsAndBytesConfig as DiffBitsAndBytesConfig from huggingface_hub import get_token import requests import torch import io def remote_text_encoder ( prompts: str | list [ str ] ): def _encode_single ( prompt: str ):
response = requests.post( "https://remote-text-encoder-flux-2.huggingface.co/predict" ,
json={ "prompt" : prompt},
headers={ "Authorization" : f"Bearer {get_token()} " , "Content-Type" : "application/json" }
) assert response.status_code == 200 , f" {response.status_code=} " return torch.load(io.BytesIO(response.content)) if isinstance (prompts, ( list , tuple )):
embeds = [_encode_single(p) for p in prompts] return torch.cat(embeds, dim= 0 ) return _encode_single(prompts).to( "cuda" )
repo_id = "black-forest-labs/FLUX.2-dev" quantized_dit_id = "diffusers/FLUX.2-dev-bnb-4bit" dit = Flux2Transformer2DModel.from_pretrained(
...
device = "cuda" def remote_text_encoder ( prompts: str | list [ str ] ): def _encode_single ( prompt: str ):
response = requests.post( "https://remote-text-encoder-flux-2.huggingface.co/predict" ,
json={ "prompt" : prompt},
...
use_stream= True , # low_cpu_mem_usage=True # uncomment for lower RAM usage )
pipe.to(device)
...
Unfold import torch from transformers import Mistral3ForConditionalGeneration from diffusers import Flux2Pipeline, Flux2Transformer2DModel from diffusers.utils import load_image
...
prompt = "the boxer kangaroo from image 1 and the martial artist turtle from image 2 are fighting in an epic battle scene at a beach of a tropical island, 35mm, depth of field, 50mm lens, f/3.5, cinematic lighting" image = pipe(
prompt=prompt,
...
Aside for the added control, this also allows for flexibility in changing specific attributes while maintaining others overall the same. For example, let's start with this json as the base schema (taken from the [official FLUX.2 prompting guide](https://docs.bfl.ai/guides/prompting_guide_flux2) ):
...
Unfold to check details on the memory-saving techniques used: Remote Text Encoder: to leverage the remote text encoding for training, simply pass --remote_text_encoder . Note that you must either be logged in to your Hugging Face account ( hf auth login ) OR pass a token with --hub_token . CPU Offloading: by passing --offload the vae and text encoder to will be offloaded to CPU memory and only moved to GPU when needed. Latent Caching: Pre-encode the training images with the vae, and then delete it to free up some memory. To enable latent_caching simply pass --cache_latents . QLoRA : Low Precision Training with Quantization - using 8-bit or 4-bit quantization. You can use the following flags: FP8 training with torchao : enable FP8 training by passing --do_fp8_training .
...
--seed= "0"
## Resources
FLUX.2 [announcement post](https://bfl.ai/blog/flux-2) Diffusers [documentation](https://huggingface.co/docs/diffusers/main/en/api/pipelines/flux2) FLUX.2 official [demo](https://huggingface.co/spaces/black-forest-labs/FLUX.2-dev) FLUX.2 on the [Hub](https://hf.co/black-forest-labs/FLUX.2-dev) FLUX.2 original [codebase](https://github.com/black-forest-labs/flux2) HTML_TAG_END
More Articles from our Blog
[guide diffusers quantization
## (LoRA) Fine-Tuning FLUX.1-dev on Consumer Hardware
+1 99 June 19, 2025 derekl35, et. al.](/blog/flux-qlora) [guide diffusers quantization
## Exploring Quantization Backends in Diffusers
derekl35, et. al. 45 May 21, 2025 derekl35, marcsun13, et. al.](/blog/diffusers-quantization)
### Community
[charchits7](/charchits7) [Nov 25, 2025](#6925dadf943f98ee0588027a)
Amazing! thanks team.
See translation ❤️ 10 10 + Reply [MaziyarPanahi](/MaziyarPanahi) [Nov 25, 2025](#69260c39763efbaa0e7259f7)
amazing! great work! 👏 is there a support for multi-gpus? (device_map=auto)
See translation 2 replies · 👍 1 1 + [sayakpaul](/sayakpaul) Article author [Nov 26, 2025](#6926b8f3bf5c52ab85acc28a)
You should be able to incorporate that in different forms. Check this out: [https://huggingface.co/docs/diffusers/main/en/training/distributed_inference](https://huggingface.co/docs/diffusers/main/en/training/distributed_inference)
See translation 🤗 2 2 + Expand 1 reply [muntedslunt](/muntedslunt) [Nov 26, 2025](#6926ae9ce74a6113cd906800)
lol... nope
1 reply · 😔 1 1 + [sayakpaul](/sayakpaul) Article author [Nov 26, 2025](#6926b9026fb402142b1885ea)
What's that supposed to mean?
See translation [NhuGiap](/NhuGiap) [Nov 26, 2025](#6926df57f265b7e9efc9e8e4)
Hi, can you tell me a bit about your motivations behind omitting all bias parameters in network architecture? Thanks!
See translation 1 reply · [sayakpaul](/sayakpaul) Article author [Nov 26, 2025](#6926e3c2f4cfb3d76eecff92)
That's a question for the Black Forest Labs team, not us.
See translation ➕ 2 2 + deleted [Nov 26, 2025](#69270f150706eeebd5b81399) This comment has been hidden [guilhermevaz](/guilhermevaz) [Nov 26, 2025](#692727cacfcedf38b072c769)
Amazing work! Can you tell me when the depth-maps model will be released? Has anyone already tried giving a depth map as a normal image? How does the model behave?
See translation Reply [sonic74](/sonic74) [Nov 26, 2025](#692746e2cfcedf38b072c77f) • [edited Nov 27, 2025](#692746e2cfcedf38b072c77f)
[https://github.com/huggingface/blog/blob/main/flux-2.md?plain=1#L283](https://github.com/huggingface/blog/blob/main/flux-2.md?plain=1#L283) causes
transformer_id, subfolder="transformer", torch_dtype=torch_dtype, device_map="cpu"
^^^^^^^^^^^^^^
NameError: name 'transformer_id' is not defined See translation 13 replies · [ariG23498](/ariG23498) Article author [Nov 27, 2025](#6927c3288b9bc560603d668d)
Probably an installation error?
pip install git+https://github.com/huggingface/diffusers -U should help you with this.
See translation Expand 12 replies [Adenialzz](/Adenialzz) [Nov 27, 2025](#6927f2cacfcedf38b072c796)
Hi, How can I deploy a text encoder privately?
See translation 3 replies · 😎 1 1 🔥 1 1 🚀 1 1 + [OzzyGT](/OzzyGT) Article author [Nov 27, 2025](#6927f929bf5c52ab85acc2ac)
Hi, you can read this [nice repo](https://github.com/ariG23498/custom-inference-endpoint) with the process that [@ ariG23498](/ariG23498) made
See translation Expand 2 replies Edit Preview Upload images, audio, and videos by dragging in the text input, pasting, or clicking here . Tap or paste here to upload images Comment
· [Sign up](/join?next=%2Fblog%2Fflux-2) or [log in](/login?next=%2Fblog%2Fflux-2) to comment
[Upvote 180](/login?next=%2Fblog%2Fflux-2) +168 System theme Company [TOS](/terms-of-service) [Privacy](/privacy) [About](/huggingface) [Careers](https://apply.workable.com/huggingface/) Website [Models](/models) [Datasets](/datasets) [Spaces](/spaces) [Pricing](/pricing) [Docs](/docs) Stripe