...
# HTML_TAG_START Mixture of Experts (MoEs) in Transformers HTML_TAG_END
Published February 26, 2026 [Update on GitHub](https://github.com/huggingface/blog/blob/main/moe-transformers.md) [Upvote 97](/login?next=%2Fblog%2Fmoe-transformers) +91 [Aritra Roy Gosthipaty ariG23498 Follow](/ariG23498) [Pedro Cuenca pcuenq Follow](/pcuenq) [merve merve Follow](/merve) [Ilyas Moutawwakil IlyasMoutawwakil Follow](/IlyasMoutawwakil) [Arthur Zucker ArthurZ Follow](/ArthurZ) [Sergio Paniego sergiopaniego Follow](/sergiopaniego) [Pablo Montalvo Molbap Follow](/Molbap) HTML_TAG_START
## [HTML_TAG_START Introduction HTML_TAG_END](#introduction) [HTML_TAG_START From Dense to Sparse: What Are MoEs? HTML_TAG_END](#from-dense-to-sparse-what-are-moes) [HTML_TAG_START Transformers and MoEs HTML_TAG_END](#transformers-and-moes) [HTML_TAG_START Weight Loading Refactor HTML_TAG_END](#weight-loading-refactor) [HTML_TAG_START Dynamic Weight Loading with WeightConverter HTML_TAG_END](#dynamic-weight-loading-with-weightconverter) [HTML_TAG_START Lazy Materialization of Tensors HTML_TAG_END](#lazy-materialization-of-tensors) [HTML_TAG_START Benchmark: Weight-Loading Pipeline Improvements HTML_TAG_END](#benchmark-weight-loading-pipeline-improvements) [HTML_TAG_START Results HTML_TAG_END](#results) [HTML_TAG_START Where Quantization Fits In HTML_TAG_END](#where-quantization-fits-in) [HTML_TAG_START Expert Backend HTML_TAG_END](#expert-backend) [HTML_TAG_START Expert Parallelism HTML_TAG_END](#expert-parallelism) [HTML_TAG_START Training MoEs with Transformers HTML_TAG_END](#training-moes-with-transformers) [HTML_TAG_START Conclusion HTML_TAG_END](#conclusion) Introduction
...
Training becomes increasingly expensive. Inference latency grows. Deployment requires significant memory and hardware.
...
[Weight Loading Refactor](#weight-loading-refactor) [Expert Backend](#expert-backend) [Expert Parallelism](#expert-parallelism) [Training MoEs with transformers](#training-moes-with-transformers)
...
WeightConverter(
[ "block_sparse_moe.experts.*.w1.weight" , "block_sparse_moe.experts.*.w3.weight" ,], "mlp.experts.gate_up_proj" ,
operations=[
...
v4 branch: [https://github.com/ariG23498/transformers/tree/bench-v4](https://github.com/ariG23498/transformers/tree/bench-v4) v5 branch: [https://github.com/ariG23498/transformers/tree/bench-v5](https://github.com/ariG23498/transformers/tree/bench-v5)
Example:
...
eager which loops over the selected experts and applies projections per expert. This is used for correctness reference and debugging.
batched_mm uses the [torch.bmm](https://docs.pytorch.org/docs/stable/generated/torch.bmm.html) API. This duplicate selected expert weights per token and performs a single batched GEMM. This backend is very well suited for small batch, GPU-heavy workloads where memory is available.
grouped_mm uses [torch._grouped_mm](https://docs.pytorch.org/docs/stable/generated/torch.nn.functional.grouped_mm.html) API. Here we sort tokens by expert ID, group them, and then perform a single grouped GEMM. This backend shines with large batches or memory-constrained setups.
| |
|---|
...
import torch from transformers import AutoModelForCausalLM, AutoTokenizer from transformers.distributed.configuration_utils import DistributedConfig
...
[GroupedGemmParallel](https://github.com/huggingface/transformers/blob/b71de73468429eb02da18caa50e9b5200400a4ed/src/transformers/integrations/tensor_parallel.py#L934) : This splits the expert weights along the expert dimension ( dim=0 ). Here each device loads only num_experts / num_devices .
...
~12× faster MoE training >35% VRAM reduction ~6× longer context 12–30× overall speedup compared to v4
...
[transformers pytorch optimization Hot
## Continuous batching from first principles
ror, et. al. 338 November 25, 2025 ror, ArthurZ, et. al.](/blog/continuous_batching) [transformers pytorch optimization
...
[Upvote 97](/login?next=%2Fblog%2Fmoe-transformers) +85 System theme Company [TOS](/terms-of-service) [Privacy](/privacy) [About](/huggingface) [Careers](https://apply.workable.com/huggingface/) Website [Models](/models) [Datasets](/datasets) [Spaces](/spaces) [Pricing](/pricing) [Docs](/docs) Stripe